...
// ============================================
// BUYER FILE REVIEW ENDPOINTS
// ============================================

// Get files for a specific file review (BUYER ACCESS)
app.get('/api/buyer/file-reviews/:reviewId/files', async (req, res) => {
  try {
    const { reviewId } = req.params;
    console.log('📥 Buyer requesting files for review:', reviewId);

    // Get review
    const reviewResult = await pool.query(
      'SELECT * FROM file_reviews WHERE id = $1',
      [reviewId]
    );
    ...
    if (reviewResult.rows.length === 0) {
      return res.status(404).json({
        success: false,
        error: 'File review not found'
      });
    }

    const review = reviewResult.rows[0];

    // Buyers can ONLY access after approval or payment
    if (review.status !== 'admin-approved' && review.status !== 'paid') {
      return res.status(403).json({
        success: false,
        error: 'Files not yet approved for download'
      });
    }

    // Get files
    const filesResult = await pool.query(
      `SELECT filename, size, mime_type as type 
       FROM file_review_files 
       WHERE review_id = $1`,
      [reviewId]
    );
...
// Download specific file (BUYER ACCESS)
app.get('/api/buyer/file-reviews/:reviewId/download/:filename', async (req, res) => {
  try {
    const { reviewId, filename } = req.params;
    const decodedFilename = decodeURIComponent(filename);

    console.log('📥 Buyer downloading file:', decodedFilename);

    // Validate review status
    const reviewResult = await pool.query(
      'SELECT status FROM file_reviews WHERE id = $1',
      [reviewId]
    );
    ...
    const fileResult = await pool.query(
      `SELECT file_data, mime_type 
       FROM file_review_files 
       WHERE review_id = $1 AND filename = $2`,
      [reviewId, decodedFilename]
    );
    ...
    // Correct Content-Disposition header (your previous version was broken)
    ...
    console.error('❌ Error downloading buyer file:', error);
    res.status(500).json({
      success: false,
